...
/// The Floonet logo, embedded at compile time and served at `/logo.svg`.
const FLOONET_LOGO_SVG: &str = include_str!("../assets/floonet-logo.svg");
...
async fn handle_web_request(
    ...
    authority: Option<Arc<name_authority::Authority>>,
) -> Result<Response<Body>, Infallible> {
    // Own the path so arms can consume `request` freely.
    let request_path = request.uri().path().to_owned();
    // Name authority endpoints (Floonet): the authority owns everything
    // under /api/v1/ plus the NIP-05 well-known document.
    if let Some(authority) = &authority {
        if name_authority::is_authority_path(&request_path) {
            return Ok(authority.handle(request, remote_addr).await);
        }
    }
    match (
        request_path.as_str(),
        ...
        ("/", false) => {
            ...
            // Default landing page: neutral Floonet branding, logo front
            // and center, and no payment wording anywhere.
            let name = settings
                .info
                .name
                .as_deref()
                .unwrap_or("floonet-rs-relay")
                .to_owned();
            let description = settings
                .info
                .description
                .as_deref()
                .unwrap_or("A Floonet relay for the Grin community Nostr network.")
                .to_owned();
            let html = format!(
...
h1{{font-size:28px;letter-spacing:-0.5px;margin:20px 0 8px}}
p{{color:#9A988F;margin:8px 0}}
</style></head><body><main>
<img src="/logo.svg" alt="Floonet">
<h1>{name}</h1>
<p>{description}</p>
<p>Please use a Nostr client to connect.</p>
        ...
        // The Floonet logo, served for the landing page.
        ("/logo.svg", false) => Ok(Response::builder()
            .status(StatusCode::OK)
            .header("Content-Type", "image/svg+xml")
            // 1 day cache
            .header("Cache-Control", "public, max-age=86400")
            .body(Body::from(FLOONET_LOGO_SVG))
            .unwrap()),
        // GoblinPay webhook: a payment server may POST {"invoice_id": ...}
        // here to hint that an invoice was paid. The relay only forwards
        // the id to the payment thread, which re-verifies the status with
        // the GoblinPay server before admitting anything, so a forged
        ...
        ("/goblinpay", false) => {
            ...
            let invoice_id = serde_json::from_slice::<serde_json::Value>(&bytes)
                .ok()
                .and_then(|v| {
                    v.get("invoice_id")
            ...
            if let Some(invoice_id) = invoice_id {
                debug!("goblinpay webhook for invoice {invoice_id}");
                payment_tx
...
pub fn start_server(settings: &Settings, shutdown_rx: MpscReceiver<()>) -> Result<(), Error> {
    ...
    rt.block_on(async {
        ...
        // Floonet: event admission pipeline (kind whitelist keystone).
        let admission = Arc::new(Admission::from_settings(&settings));
        // Floonet: built-in name authority, if enabled.
        let authority = if settings.name_authority.enabled {
            match name_authority::Authority::new(&settings, repo.clone()) {
                Ok(a) => Some(Arc::new(a)),
                Err(e) => {
                    error!("failed to start name authority: {:?}", e);
                    std::process::exit(1);
                }
            }
        } else {
                        ...
                        match parsed {
                            Ok(WrappedEvent(e)) => {
                                ...
                                if e.is_expired() {
                                    ...
                                    // Floonet admission: the composed policy
                                    // pipeline (default-deny kind whitelist,
                                    // optional NIP-42 write gate, author
                                    // whitelist) decides before anything is
                                    // queued for persistence.
                                    match admission.check(&e, conn.auth_pubkey().map(String::as_str)) {
                                        Decision::Allow => {
                                        ...
                                        Decision::Deny { reason, auth_required } => {
                                            debug!("admission denied event (kind: {}, cid: {}): {}", e.kind, cid, reason);
                                            let notice = if auth_required {
                                                Notice::auth_required(e.id.clone(), &reason)
                                            } else {
                                                Notice::blocked(e.id.clone(), &reason)
                                            };
                                            if ws_stream.send(make_notice_message(&notice)).await.is_err() {
                                                debug!("failed to send notice, closing connection (cid: {})", cid);
                                                metrics.disconnects.with_label_values(&["send_error"]).inc();
